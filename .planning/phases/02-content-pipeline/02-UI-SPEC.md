...
> **Revision 2 (2026-08-30).** Two factual corrections after `02-RESEARCH.md` (commit `5e00f33`) built both candidate pipelines in this repo and measured the results. **The visual contract is unchanged by both** — these correct a wrong number and an over-sized description of the machinery, not a design decision:
>
> - **Shiki theme `github-light` → `github-light-high-contrast`.** The spec's own 4.5:1 floor was computed against pure white, but code sits on `--color-surface-code` (`#F5F5F5`). Measured against the real background `github-light` fails on four token scopes, including `variable` at 3.20:1 — which fails against white too. The replacement is the only GitHub-family bundled theme that clears 4.5:1 on every real code scope against the tint. The floor is now stated explicitly as *measured against the code surface*, because that omission is what let the wrong number through.
> - **"Two-renderer dispatch" → one plugin chain.** `@next/mdx` with `extension: /\.(md|mdx)$/` detects format from the file suffix, so a single configuration serves both `.md` and `.mdx`. What remains is a six-line try/catch over two import specifiers. See "How these rules get delivered" after the Prose Contract.
>
> That section also records two constraints on *how* the visual rules can be delivered (Turbopack cannot take Shiki's function-valued `transformers`; Shiki already emits `tabindex="0"`) and a forward note so Phase 6's CSP is planned against Shiki's inline token styles rather than colliding with them.
...
- Highlighter: **Shiki**, build-time only, theme **`github-light-high-contrast`**. `[DISCRETION]` — CONTEXT asked for build-time over client-side to protect Phase 1's no-CLS and performance posture. Shiki runs entirely in the rehype pass and ships zero highlighting JavaScript. Because it is a **rehype** plugin it operates on hast, downstream of the format decision, so **one plugin entry highlights both `.md` and `.mdx`** — verified in a build in this repo. Prefer `@shikijs/rehype` over `rehype-pretty-code`: this phase needs no line numbers, no line highlighting and no diff notation, which is all the latter adds.
- **Override the theme's own background to `transparent`** so `--color-surface-code` governs the block, not the theme. Shiki emits an inline `background-color` on `<pre>` by default. **It must be stripped in `mdx-components.tsx`, not in `next.config.ts`:** Turbopack requires serializable plugin options, so Shiki's function-valued `transformers` option — the obvious place to do this — cannot cross into it. The `pre` component override is also where the code block's accessible name goes (see the `<pre>` row of the Prose Contract). This is a delivery constraint on a visual rule, not a change to the rule.
- Single theme, no dual light/dark output. The site is light-only (Phase 1, still in force).
- **The contrast floor is measured against the code surface tint, not against paper.** `rgba(0,0,0,0.04)` over `#ffffff` resolves to **`#F5F5F5`**, and every token must clear **4.5:1 against that**, not against `#ffffff`. This is the specific error that put the wrong theme in the first draft of this spec: the original ≈4.6:1 figure for `github-light` was computed against pure white. Measured against `#F5F5F5`, `github-light` **fails** — comment `#6a737d` **4.42**, keyword/storage `#d73a49` **4.20**, `entity.name.tag` `#22863a` **4.24**, and `variable` `#e36209` **3.20** (which fails against white too). GitHub's light palette is tuned for `#ffffff`; any tint, however small, drops every borderline token below AA.
- **`github-light-high-contrast` is the locked replacement.** An audit of all 21 bundled light themes found it is the only GitHub-family theme where every real code scope clears 4.5:1 against `#F5F5F5` — worst real scope is `comment` `#66707b` at **4.62:1**. For comparison, `github-light-default` fails one scope, `min-light` six, `light-plus` eleven, `vitesse-light` twenty-three. If a future Shiki release moves these values, re-measure **against `#F5F5F5`** and either patch the failing hexes through Shiki's serializable `colorReplacements` map or substitute full ink — never accept a failing token.
...
| `<pre>` | Body size → 18px / **1.5**, mono | `--color-surface-code` fill, square corners, `16px 24px` padding (`md`/`lg`), `32px` (`xl`) outer margin, `overflow-x: auto`, **no wrapping** — code lines must not soft-wrap. May widen to the 52rem wide measure. Shiki `github-light-high-contrast` tokens, theme background stripped to transparent in the `pre` component override. A horizontally scrolling `<pre>` must be reachable by keyboard and must have an accessible name, or it fails WCAG 2.1.1 / 4.1.2. **Shiki already emits `tabindex="0"` itself — do not add it again;** only `role="region"` and an `aria-label` need supplying, in `mdx-components.tsx`. |
...
### How these rules get delivered

Three constraints from `02-RESEARCH.md` that bound *how* the contract above can be implemented. None of them changes a visual rule; each one closes off a plausible wrong turn.

**One plugin chain serves both formats — there is no second renderer.** The first draft of this spec described a "two-renderer dispatch", plain Markdown for `.md` and a separate MDX renderer for `.mdx`. That is larger than reality. `@next/mdx` configured with `extension: /\.(md|mdx)$/` derives the format from the file suffix itself, so **one** plugin configuration (`remark-gfm`, `remark-mdx-frontmatter`, `rehype-slug`, `@shikijs/rehype`) produces every element in the table above for both extensions — verified in a build in this repo, with a `.md` file rendering `{braces}` as literal text while an `.mdx` file in the same build rendered real components. The only thing left that resembles a dispatch is a six-line try/catch over two dynamic-import specifiers in the post route. **The `extension` option cannot be omitted** — an import without an explicit file extension builds cleanly and then fails at prerender — so nobody should "simplify" it away.

**Shiki's inline `<pre>` background is stripped in `mdx-components.tsx`, not in `next.config.ts`.** Turbopack requires serializable plugin options and cannot accept Shiki's function-valued `transformers`, which is the otherwise-obvious place to do this. The `pre` component override is therefore the single place that strips the inline `background-color` *and* adds `role="region"` + `aria-label`. Shiki already emits `tabindex="0"`; adding it a second time is the error to avoid.

**Forward note for Phase 6 (`BUILD-04`):** Shiki emits a `style="color:#…"` attribute on **every** token span. A CSP with `style-src 'self'` — the natural reading of "real security response headers" — blocks inline style *attributes* under CSP Level 2+, and every code block on the site would render as undifferentiated ink the moment those headers ship. This is a known open Shiki issue (shikijs/shiki#671) with no clean fix: per-attribute hashes are impractical and `'unsafe-hashes'` is itself a weakening. Recorded here, in the phase that creates the dependency, so Phase 6 plans against it rather than discovering it. The pragmatic resolution is `style-src 'self' 'unsafe-inline'` — this site has no user input, no forms and no third-party scripts, so the XSS surface that directive guards is essentially nil — but that is Phase 6's decision to make deliberately, and the visible symptom if it is missed is monochrome code blocks.
...
New third-party dependencies this phase introduces are ordinary npm packages, not registry blocks, and are outside the registry gate's scope: `shiki` / `@shikijs/rehype` (build-time highlighting), `@next/mdx` + `@mdx-js/loader` (the Markdown/MDX loader), `remark-gfm`, `remark-mdx-frontmatter`, `rehype-slug`. Version selection and vetting belong to `02-RESEARCH.md`, which has done it.
...
| **Scoped exemption** for Shiki token hues inside `<pre>` only, theme `github-light-high-contrast` | Bounded, on the same footing as a chart's data palette. Revision 2: theme corrected after the contrast floor was re-measured against the `#F5F5F5` code surface rather than paper. |
